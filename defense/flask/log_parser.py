...
class LogParser:
    """eve.json (JSON Lines) 파서 – Suricata 알럿 로그를 파이썬 객체로 반환."""

    def __init__(self, path: str = EVE_JSON_PATH):
        self.path = os.path.abspath(path)

    def parse(self) -> list[dict]:
        """eve.json 전체를 파싱해 alert 이벤트만 반환 (최신 순)."""
        alerts: list[dict] = []
    ...
    def get_stats(self) -> dict:
        ...
        for a in alerts:
            ...
            try:
                ...
                time_key = (a["timestamp"] or "")[:16]
            timeline[time_key] += 1

        # severity 1(긴급)=3점, 2(높음)=2점, 3(낮음)=1점
        risk_score = (
